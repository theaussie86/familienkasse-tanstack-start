...
export async function migrateAccounts(
  ...
  for (const sourceAccount of sourceAccounts) {
    const accountData: NewFamilienkasseAccount = {
      id: sourceAccount.id,
...
export async function migrateTransactions(
  ...
  for (const sourceTx of sourceTransactions) {
    const txData: NewFamilienkasseTransaction = {
      id: sourceTx.id,
 ...
 */
export async function validateBalances(): Promise<{ valid: boolean; errors: string[] }> {
 ...
 */
export async function runMigration(targetUserId: string): Promise<MigrationResult> {
  const errors: string[] = [];
  let sourceClient: ReturnType<typeof postgres> | null = null;
